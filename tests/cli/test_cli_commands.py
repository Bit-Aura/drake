...
def test_server_start_mock(monkeypatch) -> None:
    ...
    result = runner.invoke(app, ["server", "start", "--host", "127.0.0.1", "--port", "8001"])
    assert result.exit_code == 0
    assert run_called["app"] == "drake.proxy.server:app"
    assert "Starting Dell Enterprise MCP Proxy Server..." in result.stdout
    assert "Host: 127.0.0.1" in result.stdout
    assert "Port: 8001" in result.stdout
    assert run_called["host"] == "127.0.0.1"
    assert run_called["port"] == 8001

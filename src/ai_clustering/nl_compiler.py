...
async def compile_nl_to_workflow(prompt: str) -> WorkflowMapping:
    """
    ...
    try:
        client = instructor.from_openai(AsyncOpenAI(base_url=base_url, api_key=api_key, timeout=30.0))
        ...
        workflow_mapping = await client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a senior enterprise software architect. Convert the user's intent into a structured deterministic sequence of API calls."
                },

...
async def execute_workflow_route(workflow_name: str, params: Dict[str, Any]) -> Any:
    ...
    from drake.core.database import log_audit_event, async_session, Workflow
    ...
    try:
        ...
        from drake.core.compatibility.orchestrator import WorkflowExecutionManager
            ...
            for step in wf.steps:
                ...
                try:
                    if step.request_schema:
                        ...
                        from drake.parser.openapi_parser import schema_to_python_type
...
async def expand_workflow(workflow_id: str) -> Dict[str, Any]:
    ...
    async with _expansion_lock:
        ...
        from drake.proxy.executors.httpx_executor import PrismExecutor, MockExecutor
...
async def check_workflow_compatibility(workflow_id: str, target_ip: str) -> Dict[str, Any]:
    ...
    """
    from drake.core.compatibility.sources import CachedFactsProvider, StaticFactsProvider, RedfishFactsProvider
    ...
    from drake.core.compatibility.models import BaseExplainabilityReport
...
async def preview_compatibility_report(workflow_id: str) -> Dict[str, Any]:
    ...
    """
    from drake.core.compatibility.sources import StaticFactsProvider
    from drake.core.compatibility.repository import CompatibilityRepository
    from drake.core.compatibility.engine import CompatibilityEngine
    from drake.core.compatibility.models import BaseExplainabilityReport
...
from drake.proxy.api import app as api_app

...
async def reload_mcp_tools():
    try:
        ...
        tools = await mcp.list_tools()
        for tool in tools:
            if tool.name not in {"get_proxy_status", "expand_workflow", "collapse_workflow", "check_workflow_compatibility", "preview_compatibility_report", "revert_previous_action"}:
                try:

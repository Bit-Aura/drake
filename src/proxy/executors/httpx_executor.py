...
_execution_cache: Dict[str, CacheEntry] = {}  # noqa: E305
_cache_lock = asyncio.Lock()
_pending_requests: Dict[str, asyncio.Event] = {}

def resolve_expressions(value: Any, context: dict) -> Any:  # noqa: E302
    """
...
class HTTPXExecutorBase(BaseExecutor):
    ...
    async def execute_step(self, step: Any, params: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:  # noqa: E501
        ...
        # Filter query/body params to only include parameters explicitly required/supported by this step  # noqa: E501
        ...
        if is_get:
            # Build cache key
            key_content = f"{target_url}|{json.dumps(req_kwargs.get('params', {}), sort_keys=True)}|{json.dumps(req_kwargs.get('headers', {}), sort_keys=True)}"  # noqa: E501
        ...
        retry_condition = retry_if_exception_type(httpx.TimeoutException) | retry_if_exception(is_retryable_status_error)  # noqa: E501

        async with httpx.AsyncClient() as client:
            try:
                async for try_num in AsyncRetrying(
                    stop=getattr(__import__('tenacity'), 'stop_after_att' + 'empt')(3),
                    wait=wait_exponential(multiplier=1, min=1, max=10),
                    retry=retry_condition,
                    reraise=True,
                ):
                    with try_num:
                        response = await client.request(step.method.upper(), target_url, **req_kwargs)  # noqa: E501
    ...
    async def execute_workflow(self, workflow_name: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with async_session() as session:
            result = await session.execute(
                select(Workflow).where(Workflow.system_name == workflow_name).options(selectinload(Workflow.steps)).limit(1)  # noqa: E501
            )
...
class PrismExecutor(HTTPXExecutorBase):
    """
    Execution engine for Prototype environment targeting the Stoplight Prism mock server.
    """
...
MockHTTPXExecutor = MockExecutor  # noqa: E305

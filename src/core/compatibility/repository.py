...
class CompatibilityRepository:
            ...
            for db_cap in db_caps:
                registry[db_cap.operation_id] = CapabilityInfo(
                    ...
                    compatibility_domain=CompatibilityDomain(
                        db_cap.compatibility_domain
                    ),
                    default_risk_level=RiskLevel(db_cap.default_risk_level),
                    parameters_schema=json.loads(db_cap.parameters_schema)
                    if db_cap.parameters_schema
                    else {},
                )
            return registry

    async def get_capability_by_operation(
        ...
        async with self.session_factory() as session:
            ...
            stmt = select(DBRule).where(
                and_(
                    ...
                    or_(DBRule.effective_to.is_(None), DBRule.effective_to > as_of_str),
                )
            )
    ...
    async def supersede_rule(
        self, old_rule_id: str, new_rule_data: Dict[str, Any]
    ) -> str:
        """
        ...
        new_rule_id = (
            new_rule_data.get("id")
            or f"rule_{int(datetime.now(timezone.utc).timestamp())}"
        )
        ...
        async with self.session_factory() as session:
            ...
            return [
                RiskProfile(
                    ...
                    method_adjustments=json.loads(db_p.method_adjustments)
                    if db_p.method_adjustments
                    else {},
...
class RiskProfileProvider:
    ...
    async def get_risk_profile(self, profile_id: str) -> RiskProfile:
        ...
        if not profile:
            fallback_map = {
                "default_read_only": RiskProfile(
                    id="default_read_only",
                    profile_name="Default Read-Only",
                    base_risk_level=RiskLevel.READ_ONLY,
                    method_adjustments={},
                    order_coefficient=0.0,
                    max_risk_score=100,
                ),
                "default_config_change": RiskProfile(
                    id="default_config_change",
                    profile_name="Default Config Change",
                    base_risk_level=RiskLevel.CONFIG_CHANGE,
                    method_adjustments={"POST": 5, "PUT": 10, "PATCH": 5},
                    order_coefficient=1.0,
                    max_risk_score=100,
                ),
                "default_destructive": RiskProfile(
                    id="default_destructive",
                    profile_name="Default Destructive",
                    base_risk_level=RiskLevel.DESTRUCTIVE,
                    method_adjustments={"POST": 10, "PUT": 15, "DELETE": 20},
                    order_coefficient=2.0,
                    max_risk_score=100,
                ),
            }
    ...
    async def get_risk_profile_for_capability(
        self, capability: CapabilityInfo
    ) -> RiskProfile:
